...
命中后立刻删号并重新鉴权换账号重试（上限 3 次账号切换），调用方感知不到这次切换。
删号原因（截断到 120 字符）写进 `/manage` 底部的「自动删号记录」。
...
1. **密钥 → 账号**：已绑定且账号仍可用（分组启用 + 账号可用 + 名下有健康 API）时保持粘性；否则重新挑选，排序依次为
...
接口级冒烟，37 项断言（Windows；会重置本地数据并留下一套演示数据）：
...
Go 测试覆盖：账号构建与余额判定、请求时刷新窗口、5 个 API 上限、分组隔离与启停、
密钥到账号的均摊分配、账号内不跨账号轮转、余额耗尽自动删号与流量故障转移、
上游「余额不足」与「预扣费额度失败」两类文案的删号换号、无限额度账号不打上游、
