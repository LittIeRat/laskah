...
推理模型常把思维链放在 `reasoning_content`（部分站点用 `reasoning`）而把 `content` 留空。
上游对这部分照样按输出 token 收费，因此本站：

- 把它计入本地输出 token，避免出现「正文空白却扣了钱」的漏计。
- 非流式一并按硬上限裁剪，不让它绕过 `max_tokens` 兜底。
- 出站转 Anthropic 时放进独立的 `thinking` 块（流式为 `thinking_delta`，与正文分属不同
  `index`），客户端不会把思维链当成正式回答渲染。
...
| 超时时间 | 额度查询**单次请求**的超时秒数，1–300，默认 30。站点挂在 Cloudflare 后面时别填太小 |
...
「超时时间」是**每次尝试各自的**上限，不是三步共享一个总预算：`/api/status` 探测另有 8 秒上限，
它卡住只会让换算单位回落到默认 `500000`，不影响后面的余额查询；第 2 步超时后第 3 步仍能拿到完整的一份超时。
超时错误会写成可执行的中文提示（含被查询的地址与已等待时长），而不是原始的 `context deadline exceeded`。
历史数据里存的 10 秒会在读取时自动抬到 30 秒。
...
请求路径上的刷新**不会陪着慢额度接口一起等**：调用方最多等 `REQUEST_REFRESH_WAIT_MS`（默认 5000 毫秒），
到点先放行，查询继续在后台跑完并把结果写回，供下一次请求判定。
后台查询也不继承调用方的 context，客户端断开不会把它取消——否则慢站点永远刷不出结果，
余额一直停在旧值反而更容易把流量打到已欠费的账号上。
「创建账号」时的首次查询同样如此：等待窗口内查完就带上余额返回，没查完则先返回账号。
