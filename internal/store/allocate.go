...
func (d *Data) UsableAccounts(groupID string) []*Account {
	...
	for _, account := range d.Accounts {
		...
		if account.Usable() && d.HealthyAccountProviders(account.ID) > 0 {
...
func (d *Data) AssignAccount(key *APIKey) *Account {
	...
	pool := d.UsableAccounts(key.GroupID)
	if len(pool) == 0 {
		return nil
	}
	...
	sort.SliceStable(pool, func(i, j int) bool {
		...
		leftShare, rightShare := d.accountKeyPressure(left.ID), d.accountKeyPressure(right.ID)
...
func (d *Data) accountKeyPressure(accountID string) float64 {
	healthy := d.HealthyAccountProviders(accountID)
	if healthy <= 0 {
		return float64(1 << 20)
	}
